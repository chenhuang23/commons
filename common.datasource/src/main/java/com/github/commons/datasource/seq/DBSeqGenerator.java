...
public class DBSeqGenerator implements SeqGenerator<Long> {
        ...
        synchronized (key.intern()) {
            try {
                ...
                while (retryLoop-- > 0) {
                    ...
                    ResultSet queryResult = query(connection, key);
                    ...
                    if (update(connection, newSeq, key, seq) == 1) {
                        // 成功才更新所有值
                        seqBase = newBase;
                        index.set(0);
                        return newSeq;
    ...
    private ResultSet query(final Connection connection, final String key) throws SQLException {

        String querySelect = MessageFormat.format(SELECT_SEQ, tableName);

        checkDataSource(connection);
    ...
    private int update(final Connection connection, final long newSeq, final String key,
    ...
    private void checkDataSource(final Connection connection) throws SQLException {
        if (connection == null) {
            // 生成失败就抛异常
