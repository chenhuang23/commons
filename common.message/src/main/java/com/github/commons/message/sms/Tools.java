package com.github.commons.message.sms;
...
//	    {
			...
			if(tmBytes == null)return null;
			byte[] destNum = new byte[tmBytes.length + 1];
			for (int i=0; i<tmBytes.length; i++) destNum[i] = tmBytes[i];
...
*/	}
	...
	public static byte[] StrToHex(String pStr)
	{
		...
		while (i < bytes.length - 1)
		{
			...
			try 
			{
				...
				if(tmpBytes[0]>=NUMBER_MIN && tmpBytes[0]<=NUMBER_MAX) tmpBytes[0] = new Integer(tmpBytes[0] - NUMBER_KEY).byteValue();
				if(tmpBytes[0]>=UPPER_MIN && tmpBytes[0]<=UPPER_MAX) tmpBytes[0] = new Integer(tmpBytes[0] - UPPER_KEY).byteValue();
				if(tmpBytes[0]>=LOWER_MIN && tmpBytes[0]<=LOWER_MAX) tmpBytes[0] = new Integer(tmpBytes[0] - LOWER_KEY).byteValue();
				...
				if(tmpBytes[1]>=NUMBER_MIN && tmpBytes[1]<=NUMBER_MAX) tmpBytes[1] = new Integer(tmpBytes[1] - NUMBER_KEY).byteValue();
				if(tmpBytes[1]>=UPPER_MIN && tmpBytes[1]<=UPPER_MAX) tmpBytes[1] = new Integer(tmpBytes[1] - UPPER_KEY).byteValue();
				if(tmpBytes[1]>=LOWER_MIN && tmpBytes[1]<=LOWER_MAX) tmpBytes[1] = new Integer(tmpBytes[1] - LOWER_KEY).byteValue();
	...
	public static String HexToStr(byte[] pBytes)
	{
		...
		for(int i = 0; i<pBytes.length; i++)
		{
			...
			if(tmpInt<0)tmpInt = tmpInt + 256;
			byte[] strList = new byte[2];
			strList[1] = new Integer(tmpInt%16).byteValue();
			strList[0] = new Integer((tmpInt/16)%16).byteValue();

			if(strList[1]>9 && strList[1]<16) strList[1] += UPPER_KEY;
			if(strList[1]>=0 && strList[1]<10) strList[1] += NUMBER_KEY;

			if(strList[0]>9 && strList[0]<16) strList[0] += UPPER_KEY;
			if(strList[0]>=0 && strList[0]<10) strList[0] += NUMBER_KEY;
